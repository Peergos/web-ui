                ...
                getRootKeyEntryFromCacheProm().thenApply(function (rootKeyPair) {
                    if (rootKeyPair != null) {
                        ...
                        directGetEntryDataFromCacheProm(rootKeyPair.username).thenApply(function (entryPoints) {
                            if (entryPoints == null) {
                                toast.error("Legacy accounts can't stay logged in. Please change your password to upgrade your account", {autoClose:false, toastId: 'login'})
                            } else {
			...
			peergos.shared.user.UserContext.signIn(
				...
				.exceptionally(function (throwable) {
                    ...
                    if (throwable.getMessage().startsWith('Invalid+TOTP+code')) {
                        toast.error('Invalid Multi Factor Authenticator code', {autoClose:false, toastId: 'login'})
                    } else {
					    toast.error(that.uriDecode(throwable.getMessage()), {autoClose:false, toastId: 'login'})
					}
				});
