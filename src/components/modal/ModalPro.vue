	...
	methods: {
		...
		this.context.requestSpace(bytes)
			...
			if (quotaBytes >= bytes && bytes > 0) {
			    ...
			    toast.info('Thank you for signing up to a paid Peergos account!',{autoClose:false, toastId: 'pro'})
			} else if (bytes == 0) {
			    that.updatePayment()
			    that.$store.commit("SET_MODAL", false)
			    toast.error("Sorry to see you go. We'd love to know what we can do better. Make sure to delete enough data to return within your Basic quota. You will revert to the Basic quota at the end of the billing month.", {autoClose:false, toastId: 'pro'})
			} else if (quotaBytes < bytes && bytes > 0 ) {
                            that.updatePayment(() => {
                                that.updateError()
                                if (! that.paymentProperties.hasError())
			            toast.error(`Card details required. Add a payment card to complete your upgrade. `,{autoClose:false, toastId: 'pro', position: 'bottom-left'})
                            });
			} else
		...
		if (this.paymentProperties.hasError()) {
		    toast.error(this.paymentProperties.getError(),{autoClose:false, toastId: 'payment', position: 'bottom-left'})
		}
	    },
