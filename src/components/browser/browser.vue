    ...
    <div class="modal-mask-app" @click="closeApp">
        <div class="modal-container full-height" @click.stop style="width:100%;overflow-y:auto;padding:0;display:flex;flex-flow:column;">
            <div class="modal-header-app">
              <span>
                <h2 v-if="!showBookmarks && toolbarDisplayable">
                    {{ addressBarText }}
            ...
            <div class="modal-body" style="padding:0;display:flex;flex-grow:1;">
                ...
                <iframe v-if="!showBookmarks" id="browserId" :src="frameUrl()" style="width:100%; min-height:100vh" frameBorder="0"></iframe>
                <div v-if="showBookmarks">
                    <div>
                        <center>
                            <input type="text" v-model="navigateToAddress" v-on:keyup.enter="navigateTo" type="text" size="60" maxlength="70" class="form-control" style="width: 600px;" placeholder="Type address">
...
module.exports = {
    ...
    created: function() {
        ...
        if (!this.supportsStreaming()) {
            ...
            let pathToFile = null;
            if (props.secretLink) {
                //todo currentFilename is not available
                let path = this.getPath;
                pathToFile = path + (path.endsWith("/") ? "" : "/") + this.currentFilename;
            } else {
                let path = props.path;
                pathToFile = path + (path.endsWith("/") ? "" : "/") + props.filename;
            }

            peergos.shared.user.App.init(that.context, "browser").thenApply(browserApp => {
                that.browserApp = browserApp;
                if (pathToFile.endsWith('/')) {
                    that.showBookmarks = true;
