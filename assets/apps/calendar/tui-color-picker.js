...
module.exports = function (context) {
  ...
  var layout = ['<ul class="{{cssPrefix}}clearfix">', '{{each preset}}', item, '{{/each}}', '</ul>', '<input type="button" class="{{cssPrefix}}palette-toggle-slider" value="{{detailTxt}}" />', '<div class="color-box"></div><div style="padding-top:10px;"><input id="color-picker-cancel-btn" type="button" class="{{cssPrefix}}palette-custom-button" value="Cancel" />', '<input id="color-picker-confirm-btn" type="button" class="{{cssPrefix}}palette-custom-button button-confirm" value="OK" /></div></div>', '</div>'].join('\n');
