...
self.onfetch = event => {
    ...
    if (event.request.mode === 'navigate') {
        ...
        // Scripts and stylesheets. Revalidated for the same reason the document
        // is: a fresh index.html paired with a previous build's calendar.js is
        // as broken as the other way round. Unchanged assets still cost a 304.
        event.respondWith(caches.open(cacheName).then(cache => {
            return fetch(event.request.url, {cache: 'no-cache'}).then(fetchedResponse => {
