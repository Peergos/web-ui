 ...
 *  share dialog makes, and a browser with no account opens it in the calendar. The owner's
 *  entry is on the grid, the calendar carries the read-only mark, its entries offer no edit,
 *  and a save forged from inside that frame - the one way round the app's own restraint - puts
 *  nothing in the owner's directory, because the host has no write access to grant.
 *
...
public class CalendarReadOnlyTest {
    ...
    public static void run(String[] args) throws Exception {
        ...
        try (WebDriver owner = Browsers.launch(Browsers.engine(engine), downloads, headless);
             WebDriver guest = Browsers.launch(Browsers.engine(engine), Temp.directory("peergos-calendar-guest-"), headless)) {
            owner.navigate(url + "/");
            owner.waitForScript("login form", "document.querySelector('input[name=username]')", 60_000);
            Page.login(owner, Server.USERNAME, Server.PASSWORD);
            CalendarApp.open(owner);
            CalendarApp.monthView(owner);
            String calendar = CalendarApp.directoryOf(owner, Server.USERNAME);

            CalendarApp.newEntry(owner, "add-menu-event", "event-modal-backdrop");
            CalendarApp.inFrame(owner, CalendarApp.setField("event-title", TITLE) + "return 1;");
            CalendarApp.save(owner, "event-save", "event-modal-backdrop");
            java.time.LocalDate today = java.time.LocalDate.now();
            String month = calendar + "/" + today.getYear() + "/" + today.getMonthValue();
            CalendarApp.awaitFileSaying(owner, month, "SUMMARY:" + TITLE);
            ...
            owner.script("window.__link = null; window.__linkErr = null;"
            ...
            owner.waitForScript("the link", "window.__link || window.__linkErr", 120_000);
            Object linkError = owner.script("return window.__linkErr");
            if (linkError != null)
                throw new IllegalStateException("Could not make the link: " + linkError);
            String link = String.valueOf(owner.script("return window.__link"));
            System.out.println("read-only link made");

            guest.navigate(url + "/" + link + "?open=true");
            guest.waitForScript("the linked folder", "!!document.querySelector('#app') && !!document.querySelector('#app').__vue__"
            ...
            guest.script("let stack = [document.querySelector('#app').__vue__];"
                    + "while (stack.length) {"
                    + "  let c = stack.pop();"
            ...
            guest.waitForScript("the calendar view", "(() => {"
                    + "  let root = null;"
            ...
            guest.waitForScript("the calendar frame", "!!document.querySelector('" + CalendarApp.FRAME + "')", 60_000);
            CalendarApp.waitInFrame(guest, "the calendar app to load",
            ...
            CalendarApp.waitInFrame(guest, "the owner's entry on the guest's grid",
            ...
            System.out.println("  ok   the guest sees the owner's entry");

            boolean marked = Boolean.TRUE.equals(CalendarApp.inFrame(guest,
            ...
            boolean offered = Boolean.TRUE.equals(CalendarApp.inFrame(guest,
            ...
            if (offered)
                throw new AssertionError("The popover offers Edit on an entry the guest cannot write");
            ...
            String sharedName = String.valueOf(guest.script(
                    "return window.__cal.calendarProperties.calendars[0].name;"));
            List<String> before = CalendarApp.list(owner, month);
            CalendarApp.inFrame(guest,
                    "window.parent.postMessage({type: 'save', calendarName: " + CalendarApp.quote(sharedName)
                            ...
                            + ", isRecurring: false, isTask: false, Id: 'forged-by-guest',"
                            + " item: 'BEGIN:VCALENDAR\\r\\nBEGIN:VEVENT\\r\\nUID:forged-by-guest\\r\\n"
                            + "DTSTART:20260401T100000Z\\r\\nSUMMARY:forged\\r\\nEND:VEVENT\\r\\nEND:VCALENDAR\\r\\n'}, '*');"
                            + "return 1;");
            WebDriver.sleep(5000);
            List<String> after = CalendarApp.list(owner, month);
            if (after.contains("forged-by-guest.ics") || ! new HashSet<>(after).equals(new HashSet<>(before)))
                throw new AssertionError("A save forged in the guest's frame changed the owner's month: "
                        + before + " became " + after);
            System.out.println("  ok   a save forged in the guest's frame writes nothing in the owner's calendar");
